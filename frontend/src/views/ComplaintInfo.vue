<template>
    <el-main class="infoCi">
        <el-page-header @back="goBack" content="投诉详情">
        </el-page-header>
        <p v-for="(item,key,index) in complaintInfo" :key="key">
            {{complaintLabel[index]}}:{{item}}
        </p>  
...
export default {
    data() {
        return {
            ...
            complaintLabel:[
                '投诉编号','房源编号','投诉内容','处理状态','客服回复','评价内容','评分'
            ],
            commentForm:{
                complaintId:'',
                evalution:'',
        ...
        makeComplaintComment() {
            ...
            this.$refs.commentForm.validate(valid =>{
                if(valid) {
                    ...
                    submitComplaintComment(this.commentForm)
                    .then((res)=> {
    ...
    mounted() {
        getComplaint({complaintId:this.$route.query.complaintId})
        .then(res =>{
