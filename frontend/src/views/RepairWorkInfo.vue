<template>
    <el-main class="infoRwi">
        <el-page-header @back="goBack" content="维修详情">
        </el-page-header>
        <p v-for="(item,key,index) in repairWorkInfo" :key="key">
            {{repairWorkLabel[index]}}:{{item}}
        </p>  
...
export default {
    data() {
        return {
            ...
            repairWorkLabel:[
                '工单编号','报修编号','房源编号','房源地址','报修内容','图片','状态','回复','评价','租户手机号'
            ],
            callbackForm:{
                repairWorkId:'',
                callback:''
        ...
        makeRepairComment() {
            ...
            /*this.$refs.callbackForm.validate(valid =>{
                if(valid) {*/
                    ...
                    submitRepairWorkCallback(this.callbackForm)
                    .then((res)=> {
    ...
    mounted() {
        getRepairWork({repairId:this.$route.query.repairId})
        .then(res =>{
